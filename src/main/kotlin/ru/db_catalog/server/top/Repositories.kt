...
interface BookTopRepository : CrudRepository<BookTop, Long> {
    ...
    @Query("select id from top where id in (select top_id from top_has_book)")
    fun findAllId(): Set<Long>

    @Query("select id from top where lower(top.name) like :name and id in (select top_id from top_has_book group by top_id)" )
...
interface FilmTopRepository : CrudRepository<FilmTop, Long> {
    ...
    @Query("select id from top where id in (select top_id from top_has_film)")
    fun findAllId(): Set<Long>

    @Query("select id from top where lower(top.name) like :name and id in (select top_id from top_has_film group by top_id)" )
    fun findIdAllByNameLikeIgnoreCase(name: String) : Set<Long>
}
...
interface MusicTopRepository : CrudRepository<MusicTop, Long> {
    ...
    @Query("select id from top where id in (select top_id from top_has_music)")
    fun findAllId(): Set<Long>

    @Query("select id from top where lower(top.name) like :name and id in (select top_id from top_has_music group by top_id)" )
    fun findIdAllByNameLikeIgnoreCase(name: String) : Set<Long>
}