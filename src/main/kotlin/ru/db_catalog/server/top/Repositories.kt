...
interface BookTopRepository : CrudRepository<BookTop, Long> {
    ...
    @Query("select id, name from top where lower(top.name) like :name and id in (select top_id from top_has_book group by top_id)")
    fun findIdNameAllByNameLikeIgnoreCase(name: String): Set<ContentIdName>
}
...
interface FilmTopRepository : CrudRepository<FilmTop, Long> {
    ...
    @Query("select id, name from top where lower(top.name) like :name and id in (select top_id from top_has_film group by top_id)")
    fun findIdNameAllByNameLikeIgnoreCase(name: String): Set<ContentIdName>
}
...
interface MusicTopRepository : CrudRepository<MusicTop, Long> {
    ...
    @Query("select id, name from top where lower(top.name) like :name and id in (select top_id from top_has_music group by top_id)")
    fun findIdNameAllByNameLikeIgnoreCase(name: String): Set<ContentIdName>
